...
const uInput = document.getElementById("search");
...
function doSearch() {
	...
	const filteredRecipes = recipes.filter(recipe => {
		...
		|| recipe.author.toLowerCase().includes(userInput)
		|| recipe.datePublished.toLowerCase().includes(userInput)
		|| recipe.tags.find(tag => tag.toLowerCase().includes(userInput));
	});

	filteredRecipes.sort((a, b) => searchSort(a, b));

	document.getElementById("content").innerHTML = '';

	filteredRecipes.forEach(recipe => recipesTemplate(recipe));
}

function searchSort(a, b, i = 0) {
	let a_not = false
	let b_not = false
	...
	let dif = a.name[i] - b.name[i]
	if (dif === 0){
		return searchSort(a, b, i + 1);
	}
	else{
		return dif;
	}
		...
		<div class="recipe-card">
			...
			<div class="recipe-details">
				<div class="recipe-tags">
					${recipe.tags.map(tag => `<button><b>${tag}</b></button>`).join(' ')}
				</div>
				...
				>
				${makeRatingString(recipe.rating)}
				</span>
...
function makeRatingString(rating){
	let ratingString = ``;
...
function init(){
	recipesTemplate(recipes[randomNum]);
}
